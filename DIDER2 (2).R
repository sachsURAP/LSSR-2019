...
dfrLow=subset(dfra,Z<3)#start RKS
...
simulated_calibration=nls(HG~low_LET_IDER(beta,lambda,Dose),data=dfrLow,start=list(beta=.8,lambda=.1))#here actual calibration
...
HZE_MIXDER = function(r, L = 193, aa1 = HZEc[1], aa2 = HZEc[2], kk1 = HZEc[3], phi = 3e3, lambda = coefs[1], beta = coefs[2], d) {
  dE=function(yini,State,Pars){
    ...
    with(as.list(c(State, Pars)), {
      u = vector(length = 1)
      u[1] = uniroot(function(d) 1-exp(-0.01*(aa1*L*d*exp(-aa2*L)+(1-exp(-150*phi*d/L))*kk1)) - I, lower = 0, upper = 5, extendInt = "yes", tol = 10^-10)$root 
      dI = vector(length = 2)
...
HZE_MIXDER_IDER(r = c(0.2, 0.8), d = d)

plot(x = d, y = HZEC(Dose = d, L = 173), type = "l", xlab="dose",ylab="HG",bty='l')
lines(x = d, y = low_LET_IDER(beta = coefs[2], lambda = coefs[1], d = d))
#lines(x = d, y = HZE_MIXDER(r = c(0.2, 0.8), d = d)[ ,2], col = "blue")
lines(x = d, y = HZE_MIXDER_IDER(r = c(0.2, 0.8), d = d)[, 2], col = "green", lwd=2)
