...
# # LOW.m <- nls(HG ~ .0275 + bet * Dose^2 * exp(-lam * Dose),
# #            data = dfrL,
# #            weights = NWeight,
# #            start = list(bet = 5, lam = .2))
# # summary(LOW.m)
# # 
# # LOW.c <- coef(LOW.m)  # calibrated central values of the 2 parameters
# # LOW.C <- function(Dose, L) {
# #   return(LOW.c[1] * Dose ^2 * exp(-LOW.c[2] * Dose))
# # }
# # LOW.m <- nls(HG ~ .0275 + 1-exp(-bet * Dose),
# #              data = dfrL,
# #              weights = NWeight,
# #              start = list(bet = .5))
# # summary(LOW.m)
...
ddose=0.01*0:700; lines(ddose, LOW.C(ddose,0 )+.0275)  #calibrated lowLET IDER
points(dfrL[1:8, "Dose"], dfrL[1:8,"HG"],pch=19) #RKS: Helium data points

#lines(dfrL[9:12, "Dose"], LOW.C(dfrL[9:12, "Dose"], .4) + .0275)#no longer needed
points(dfrL[9:12, "Dose"], dfrL[9:12, "HG"] )  # proton data points 
...
dE_IDER <- function(d, beta, lambda) {
