...
beta_decay_data <- data.frame(dose = c(0, 0.4, 0.8, 1.6, 3.2, 7, 0, .4, .8, .12, 1.6),
...
clean_hze_data <- subset(clean_hg_data, Z > 3) # look only at HZE not at much lower Z and LET ions. # In next line phi controls how fast NTE build up from zero; not really needed during calibration since phi*Dose>>1 at every observed Dose !=0. phi needed for later synergy calculations.
...
calib_nte_hazard_func <- function(dose, L) { #  calibrated hazard function "calib_nte_hazard_func" is for hazard non-targeted calibrated
  0.01 * (hi_nte_model_coef[1] * L * dose * exp(-hi_nte_model_coef[2] * L) + (1 - exp(- phi * dose)) * hi_nte_model_coef[3])
} 

calib_hze_nte_ider <- function(dose, L) {
  1 - exp(-calib_nte_hazard_func(dose, L)) #  Calibrated HZE NTE IDER
}
...
bayesian_ic_table <- cbind(AIC(hi_te_model, hi_nte_model), BIC(hi_te_model, hi_nte_model))
print(bayesian_ic_table)

dose_vector <- c(seq(0, .00001, by = 0.000001), #  look carefully near zero, but go out to 0.5 Gy
...
Generate_CI <- function(N = 500, intervalLength = 0.95, d, r, L, HZEmodel = hi_nte_model, method = 0) {
  ...
  if (method) {
    ...
    for (i in 1:500) {
      valueArr = c(valueArr, calculate_id_hze_nte_mixder(r = r, L = L, d = c(0, d), aa1 = monteCarloSamples[, 1][i], aa2 = monteCarloSamples[, 2][i], kk1 = monteCarloSamples[, 3][i])[, 2][2])
    }
    ...
    upper = calculate_id_hze_nte_mixder(r = r, L = L, d = c(0, d), aa1 = meanArr["aa1"] + 2*stdErrArr["aa1"], aa2 = meanArr["aa2"] + 2*stdErrArr["aa2"], kk1 = meanArr["kk1"] + 2*stdErrArr["kk1"])[, 2][2]
    lower = calculate_id_hze_nte_mixder(r = r, L = L, d = c(0, d), aa1 = meanArr["aa1"] - 2*stdErrArr["aa1"], aa2 = meanArr["aa2"] - 2*stdErrArr["aa2"], kk1 = meanArr["kk1"] - 2*stdErrArr["kk1"])[, 2][2]
...
mixderCurve = calculate_id_hze_nte_mixder(r, L)
