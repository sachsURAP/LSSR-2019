...
MIXDER_function = function(r, L,d =dose,aa1=.7538,aa2=.00306,kk1=7.459) {
   dE=function(yini,State,Pars){
     ...
     with(as.list(c(State, Pars)), {
       ...
       for (i in 1:length(L)) {
         dI[i] = r[i]*(0.01*aa[i]+exp(-150*phi*u[i]/L)*kk1*150*phi/L)*exp(-0.01*(aa[i]*u[i]+(1-exp(-150*phi*u[i]/L))*kk1))
...
plot(INCRL, type='l')
lines(dose,HZEC(dose,200),col='green')#component #1
lines(dose,HZEC(dose,50),col='green') #component #2
#To Be Done: 1 Why all the warnings? 2. add simple effect additivity S(d) and graphs including S(d). 3. graphs at dose ranges (0,.5) and (0,0.0001)
